...
if __name__ == '__main__':
    ...
    parser.add_argument(u'--main_checkpoint_path', u'-mnp', 
                        help=u'directory to trained feed forward network.')
    parser.add_argument(u'--main_checkpoint_num_styles',
                        help=u'TODO')
    parser.add_argument(u'--style_checkpoint_paths', u'-snp', default=[],
                        help=u'directory to trained feed forward network.')
    parser.add_argument(u'--style_checkpoint_num_styles', u'-snp', default=[],
                        help=u'TODO')
    ...
    for i in range(num_styles):
        stylizer = Stylizer(args.mat_dir,128,128,args.style_checkpoint_num_styles[i], use_johnson=True, save_dir=args.style_checkpoint_paths[i], do_save_npy=True, npy_path='style_%d.npy' %(i))
        del stylizer

    main_npy = np.load('main.npy')
    style_npys = [np.load('style_%d.npy' %(i)) for i in range(num_styles)]
