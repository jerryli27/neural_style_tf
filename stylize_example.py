...
if __name__=='__main__':
    ...
    checkpoint_output_str = 'stylize_examples/output_checkpoint/7_iter_%s.jpg'
    output_str = 'stylize_examples/output/7_result.jpg'

    os.system('python neural_style.py --content=%s --styles %s --learning-rate=%f '
              '--iterations=%d --checkpoint-output=%s --output=%s --width=%d --height=%d --content-weight=%f '
              '--style-weight=%f --use_mrf --use_new_features --new_features_save_path="model/new_features_model/"'
              %(content, ' '.join(styles), learning_rate, iterations, checkpoint_output_str, output_str, width, height, content_weight, style_weight,))
    #
    # TODO: Debugging.
    #
